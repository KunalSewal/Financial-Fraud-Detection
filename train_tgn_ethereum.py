"""
Train TGN on Ethereum Dataset - Phase 1 Validation.

This script trains the Temporal Graph Network on the Ethereum fraud detection
dataset using real temporal edges. Target: Beat MLP (93.99%) and GraphSAGE (91.31%).
"""
...
def train_epoch(model, data, optimizer, criterion, device):
    ...
    # Get temporal batches (sorted by time)
    ...
    # Forward pass through all edges (temporal order)
    logits = model(
...
def train(args):
    ...
    print(f"  Edge features: {data.edge_attr.shape[1] if hasattr(data, 'edge_attr') else 0}")
    ...
    edge_dim = data.edge_attr.shape[1] if hasattr(data, 'edge_attr') else 1
    ...
    if not args.no_wandb:
        ...
        runner.init_run(
            name=f"tgn-ethereum-{args.run_name}",
            config={
                'model': 'TGN',
                'dataset': 'Ethereum',
                'num_nodes': data.num_nodes,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics['auc'] > best_val_auc:
            ...
            checkpoint_path = f'checkpoints/tgn_ethereum_best.pt'
            torch.save({
                'epoch': epoch,
            ...
            if not args.no_wandb:
                runner.log_model(checkpoint_path, f"tgn_ethereum_epoch_{epoch}")
        else:
            patience_counter += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train TGN on Ethereum')
    
    # Model hyperparameters
    parser.add_argument('--memory-dim', type=int, default=128, help='Memory dimension')
    parser.add_argument('--time-dim', type=int, default=32, help='Time encoding dimension')
